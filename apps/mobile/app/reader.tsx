  ...
  return <SafeAreaView edges={["top","bottom","left","right"]} style={[s.safe,{backgroundColor:theme.background}]}>
    <View style={s.content}>
      ...
      <ScrollView ref={scrollRef} style={s.textArea} contentContainerStyle={s.readingContent}>
        {passages.map((passage,passageIndex)=>{const current=passageIndex===index;const tokens=current?passage.split(/(\s+)/):[];return <Text key={passageIndex} onLayout={event=>measureSentence(passageIndex,event)} style={[s.sentence,readingType,{color:theme.text},current&&sentenceHighlight&&{backgroundColor:theme.sentenceHighlight}]}>{current?tokens.map((token,tokenIndex)=>{if(/^\s+$/.test(token))return token;const before=tokens.slice(0,tokenIndex).join("");const spokenIndex=before.match(/\S+/g)?.length||0;const active=spokenIndex===wordIndex;return <Text key={tokenIndex} style={active&&wordHighlight?[{backgroundColor:theme.wordHighlight,color:theme.accent,fontWeight:"900"},accessibility.wordEmphasis&&{fontSize:(readingType.fontSize as number)+2}]:undefined}>{token}</Text>;}):passage}</Text>;})}
      </ScrollView>
      ...
      <View style={[s.dock,{borderColor:theme.border,backgroundColor:theme.surface}]}>
        <View style={s.nowListeningHeader}><VoticLogo compact markOnly/><View style={s.nowListeningCopy}><Text style={[s.nowListeningLabel,{color:theme.text}]}>Now Listening</Text><Text numberOfLines={1} style={[s.nowListeningTitle,{color:theme.mutedText}]}>{activeDocument?.title||"Document"}</Text></View><Ionicons name="chevron-up" size={20} color={theme.mutedText}/></View>
        ...
        <View style={s.playbackMeta}><Text style={[s.timeText,{color:theme.mutedText}]}>{clockLabel(elapsedSeconds)} / {clockLabel(totalSeconds)}</Text><Pressable accessibilityRole="button" accessibilityLabel={`Playback speed ${formatPlaybackRate(rate)}`} onPress={()=>openSheet("listen")} style={s.rateButton}><Text style={[s.rateText,{color:theme.mutedText}]}>{formatPlaybackRate(rate)}</Text><Ionicons name="speedometer-outline" size={15} color={theme.mutedText}/></Pressable></View>
        <View accessibilityRole="progressbar" accessibilityValue={{min:0,max:100,now:Math.round(progress*100)}} style={[s.dockTrack,{backgroundColor:theme.border}]}><View style={[s.fill,{backgroundColor:theme.accent,width:`${progress*100}%` as `${number}%`}]}/></View>
