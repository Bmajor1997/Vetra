        ...
        <Pressable accessibilityRole="none" onPress={event=>event.stopPropagation()} style={[s.sheet,{backgroundColor:theme.surface}]}>
          ...
          <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={s.sheetContent}>
            ...
            {sheet==="listen"?<>
              ...
              <Setting label="Voice">{voices.length?voices.map((voice,voiceIndex)=><VoiceChoice key={voice.identifier} name={voticVoiceName(voiceIndex)} identifier={voice.identifier} selected={accessibility.voiceIdentifier===voice.identifier} previewing={previewVoiceIdentifier===voice.identifier} onPreview={()=>{if(previewVoiceIdentifier===voice.identifier)void stop();else void previewVoice(voice,voiceIndex);}} onSelect={()=>{void stop();accessibility.setVoiceIdentifier(voice.identifier);}}/>):<Text style={[s.emptyVoices,{color:theme.mutedText}]}>Your device voice will be used.</Text>}</Setting>
            </>:null}
          </ScrollView>
        </Pressable>
      </Pressable>
