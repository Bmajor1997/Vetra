    ...
    <Modal visible={completionOpen} transparent animationType={accessibility.reduceMotion?"none":"fade"} onRequestClose={()=>setCompletionOpen(false)}>
      <View style={s.completionBackdrop}>
        <View accessibilityViewIsModal style={[s.completionCard,{backgroundColor:theme.surface,borderColor:theme.border}]}>
          ...
          <Text accessibilityRole="header" style={[s.completionTitle,{color:theme.text}]}>Document complete</Text>
          <Text numberOfLines={2} style={[s.completionDocument,{color:theme.mutedText}]}>{activeDocument?.title}</Text>
          <View style={s.completionStats}><View style={s.completionStat}><Text style={[s.completionValue,{color:theme.text}]}>100%</Text><Text style={[s.completionLabel,{color:theme.mutedText}]}>Read</Text></View><View style={[s.completionDivider,{backgroundColor:theme.border}]}/><View style={s.completionStat}><Text style={[s.completionValue,{color:theme.text}]}>{passages.length}</Text><Text style={[s.completionLabel,{color:theme.mutedText}]}>Passages</Text></View></View>
          <Pressable accessibilityRole="button" accessibilityLabel="Ask Votic about this document" onPress={()=>{setCompletionOpen(false);router.push("/assistant");}} style={({pressed})=>[s.completionPrimary,{backgroundColor:theme.accent,opacity:pressed?.78:1}]}><Ionicons name="chatbubble-ellipses" size={19} color="#FFF"/><Text style={s.completionPrimaryText}>Ask Votic</Text></Pressable>
          <View style={s.completionActions}><Pressable accessibilityRole="button" accessibilityLabel="Open worksheet workspace" onPress={()=>{setCompletionOpen(false);router.push("/worksheet");}} style={({pressed})=>[s.completionSecondary,{borderColor:theme.border,opacity:pressed?.65:1}]}><Text style={[s.completionSecondaryText,{color:theme.text}]}>Worksheet</Text></Pressable><Pressable accessibilityRole="button" accessibilityLabel="Return home" onPress={()=>{setCompletionOpen(false);router.replace("/(tabs)");}} style={({pressed})=>[s.completionSecondary,{borderColor:theme.border,opacity:pressed?.65:1}]}><Text style={[s.completionSecondaryText,{color:theme.text}]}>Done</Text></Pressable></View>
        </View>
      </View>
    </Modal>
