...
export function validateImport(candidate:ImportCandidate){if(!supportedDocument(candidate.name))throw new Error("Choose a TXT, Markdown, PDF, Word, PowerPoint, or EPUB document.");if((candidate.size||0)>MAX_DOCUMENT_BYTES)throw new Error("Document is too large. The current limit is 25 MB.");return candidate}
