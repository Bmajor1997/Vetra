const HELP_TOPICS = [
  ...
  { terms: ["worksheet", "checkbox", "blank", "answer"], answer: "Choose Worksheet document during upload. Votic turns recognized checkboxes and answer blanks into controls you can complete while listening." },
  ...
  { terms: ["color", "highlight", "contrast", "personalize"], answer: "Choose the color-circle button near the top-right corner. Your selection changes highlights and Votic’s accent color; High contrast is also available." },
  { terms: ["pdf", "scan", "selectable text"], answer: "Votic reads PDFs that contain selectable text. Scanned-image PDFs still require OCR, which is planned but not included in this prototype." },
...
export function local_help_answer(question) {
  ...
  if (!normalized) return "Ask me how to upload, listen to, navigate, personalize, complete, or download a document in Votic.";
  const topic = HELP_TOPICS.find(({ terms }) => terms.some((term) => normalized.includes(term)));
  return topic?.answer || "I can help with uploading documents, playback, sections, worksheets, downloads, colors, accessibility, PDF problems, and listening reviews. Try asking about one of those parts of Votic.";
}

export const VOTIC_HELP_CONTEXT = `You are Ask Votic, a concise support assistant for the Votic document-reading prototype. Answer only questions about using Votic. Votic uploads PDF, DOCX, TXT, and Markdown files; offers Reading and Worksheet modes; reads using browser speech; highlights passages and words; has section navigation, playback speeds from 1x to 3x, a clickable progress bar, accessibility and color settings, worksheet TXT/DOCX exports, optional AI summary/key-takeaway downloads, and opt-in document questions when AI is connected. Scanned-PDF OCR, user accounts, browser extensions, and cloned voices are not implemented. Never claim a feature exists when it does not. Do not request or expose document contents, passwords, API keys, or other sensitive information. If asked about an unrelated topic, explain that you can only help with Votic.`;
