...
import { build_document, build_review, build_worksheet_export, format_playback_rate, format_time, keyboard_command, normalize_highlight_theme, normalize_resume_snapshot, parse_document_text, PLAYBACK_RATES, progress_for_sentence, section_timing, speech_segment, worksheet_control_id, worksheet_control_type } from "./model.js";
...
const PREFERENCES_KEY = "voxify.preferences.v1";
...
function apply_highlight_theme(theme) {
  ...
  try { localStorage.setItem(PREFERENCES_KEY, JSON.stringify({ highlightTheme: safe_theme })); } catch { /* Personalization still works for the current session. */ }
}
function restore_highlight_preferences() { try { apply_highlight_theme(JSON.parse(localStorage.getItem(PREFERENCES_KEY))?.highlightTheme); } catch { apply_highlight_theme("warm"); } }
...
restore_highlight_preferences(); restore_state(); render_document(); render_sections(); persistenceReady = true;
...
el.highlightTrigger.addEventListener("click", () => { pause_for_choice(); el.highlightDialog.showModal(); (el.highlightDialog.querySelector('[aria-checked="true"]') || el.highlightDialog.querySelector("[data-highlight-theme]")).focus(); announce("Highlight color options opened. Playback paused."); });
el.highlightDialog.addEventListener("click", (event) => { const option = event.target.closest("[data-highlight-theme]"); if (!option) return; apply_highlight_theme(option.dataset.highlightTheme); announce(`${option.textContent.trim()} highlight selected. Playback remains paused.`); });
