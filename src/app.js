...
function load_parsed_document(parsed, mode = "reading") { if (!parsed.sections.length) { el.documentError.textContent = "This document does not contain readable text."; return; } cancel_speech(); doc = build_document(parsed.sections); documentTitle = parsed.title; active_document_mode = mode === "worksheet" ? "worksheet" : "reading"; worksheet_responses = {}; review_options = { summary: true, takeaways: true }; state.sentenceIndex = 0; state.wordIndex = 0; state.completed = false; el.resumeCard.hidden = true; render_document(); render_sections(); render_state(); el.dialog.close(); window.scrollTo({ top: 0, behavior: "smooth" }); }
