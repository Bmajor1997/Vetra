...
el.downloadReview.addEventListener("click", () => { const review = render_review(); const body = `${documentTitle}\n\nSummary\n${review.summary}\n\nKey takeaways\n${review.takeaways.map((item) => `• ${item}`).join("\n")}\n`; const url = URL.createObjectURL(new Blob([body], { type: "text/plain;charset=utf-8" })); const link = document.createElement("a"); link.href = url; link.download = `${documentTitle.replace(/[^a-z0-9]+/gi, "-").replace(/^-|-$/g, "").toLowerCase() || "vetra"}-review.txt`; link.click(); URL.revokeObjectURL(url); });
