...
import { build_document, build_review, format_playback_rate, format_time, normalize_resume_snapshot, parse_document_text, PLAYBACK_RATES, progress_for_sentence } from "./model.js";
...
function render_sections() {
  el.sectionList.innerHTML = doc.sections.map((section, index) => `<button data-section-jump="${index}"><span>${String(index + 1).padStart(2, "0")}</span><strong>${escape_html(section.heading)}</strong><small>${section.wordCount} words</small></button>`).join("");
...
function toggle_panel(open) { if (open) pause_for_choice(); el.panel.classList.toggle("is-open", open); el.scrim.classList.toggle("is-open", open); el.panel.setAttribute("aria-hidden", String(!open)); el.trigger.setAttribute("aria-expanded", String(open)); if (open) el.close.focus(); else el.trigger.focus(); }
...
PLAYBACK_RATES.forEach((speed) => { const button = document.createElement("button"); button.role = "option"; button.textContent = format_playback_rate(speed); button.dataset.speed = speed; el.speedMenu.append(button); });
...
el.sectionList.addEventListener("click", (event) => { const button = event.target.closest("[data-section-jump]"); if (!button) return; const section = doc.sections[Number(button.dataset.sectionJump)]; state.sentenceIndex = section.sentences[0].index; state.wordIndex = 0; cancel_speech(); toggle_panel(false); render_state({ scroll: true }); });
el.speedButton.addEventListener("click", () => { pause_for_choice(); const open = !el.speedMenu.classList.contains("is-open"); el.speedMenu.classList.toggle("is-open", open); el.speedButton.setAttribute("aria-expanded", String(open)); });
el.speedMenu.addEventListener("click", (event) => { const button = event.target.closest("[data-speed]"); if (!button) return; state.rate = Number(button.dataset.speed); state.playing = false; el.speedButton.textContent = format_playback_rate(state.rate); el.speedMenu.classList.remove("is-open"); el.speedButton.setAttribute("aria-expanded", "false"); render_state(); });
el.playPause.addEventListener("click", () => state.playing ? cancel_speech() : speak_current_sentence()); el.previous.addEventListener("click", () => jump_to_sentence(state.sentenceIndex - 1)); el.next.addEventListener("click", () => jump_to_sentence(state.sentenceIndex + 1));
...
el.openReview.addEventListener("click", () => { render_review(); el.reviewDialog.showModal(); el.closeReview.focus(); }); el.closeReview.addEventListener("click", () => el.reviewDialog.close());
...
document.addEventListener("keydown", (event) => { if (event.key === "Escape") { if (el.panel.classList.contains("is-open")) toggle_panel(false); el.speedMenu.classList.remove("is-open"); el.speedButton.setAttribute("aria-expanded", "false"); } });
render_state();
