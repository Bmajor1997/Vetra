import { build_document, build_review, build_worksheet_export, format_playback_rate, format_time, keyboard_command, location_for_progress, normalize_highlight_theme, normalize_resume_snapshot, parse_document_text, PLAYBACK_RATES, progress_for_sentence, section_timing, speech_segment, worksheet_control_id, worksheet_control_type } from "./model.js";
let doc = null;
let documentTitle = "";
...
Object.assign(el, { accessibilityTrigger: $("#accessibilityTrigger"), accessibilityDialog: $("#accessibilityDialog"), closeAccessibility: $("#closeAccessibility"), doneAccessibility: $("#doneAccessibility"), reduceMotion: $("#reduceMotion"), wordEmphasis: $("#wordEmphasis"), closeDocumentTrigger: $("#closeDocumentTrigger"), closeDocumentDialog: $("#closeDocumentDialog"), cancelCloseDocumentIcon: $("#cancelCloseDocumentIcon"), cancelCloseDocument: $("#cancelCloseDocument"), confirmCloseDocument: $("#confirmCloseDocument"), assistantTrigger: $("#assistantTrigger"), assistantPanel: $("#assistantPanel"), assistantScrim: $("#assistantScrim"), closeAssistant: $("#closeAssistant"), assistantMessages: $("#assistantMessages"), assistantForm: $("#assistantForm"), assistantQuestion: $("#assistantQuestion"), sendAssistant: $("#sendAssistant") });
...
function show_reader(open) { el.emptyState.hidden = open; el.reader.hidden = !open; el.player.hidden = !open; el.trigger.hidden = !open; el.closeDocumentTrigger.hidden = !open; document.body.classList.toggle("has-document", open); }
...
restore_preferences(); const restored_document = restore_state(); show_reader(restored_document); if (restored_document) { render_document(); render_sections(); } persistenceReady = true;
...
el.downloadWorksheetDocx.addEventListener("click", async () => {
  ...
  try {
    const response = await fetch("/api/export-docx", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ title: documentTitle, content: build_worksheet_export(doc, worksheet_responses) }) });
...
el.cancelCloseDocumentIcon.addEventListener("click", cancel_close_document); el.cancelCloseDocument.addEventListener("click", cancel_close_document); el.confirmCloseDocument.addEventListener("click", close_current_document);
...
function add_assistant_message(text, role) { const message = document.createElement("div"); message.className = `assistant-message assistant-message-${role}`; message.textContent = text; el.assistantMessages.append(message); message.scrollIntoView({ block: "end" }); return message; }
async function ask_assistant(question) {
  ...
  try { const response = await fetch("/api/help", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ question }) }); const result = await response.json(); if (!response.ok) throw new Error(result.error || "Vetra Help could not answer right now."); pending.textContent = result.answer; }
...
el.assistantPanel.querySelector(".assistant-suggestions").addEventListener("click", (event) => { const button = event.target.closest("button"); if (!button) return; ask_assistant(button.textContent); });
